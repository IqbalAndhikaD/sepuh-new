...
class signInScreenUser extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    Future<void> _login() async {
      final response = await http.post(
        Uri.parse('https://sepuh-api.vercel.app/user/login/pasien'),
        headers: {
      ...
      if (response.statusCode == 200 && jsonData['status'] == 200) {
        ...
        Navigator.pushReplacement(
          context,
          MaterialPageRoute(builder: (context) => const botNavbarUser(index: 0)),
        );
      } else {
